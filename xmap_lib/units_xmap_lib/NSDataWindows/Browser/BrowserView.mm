...
bGenericMacMapApp*   gapp=[self app];
    if(!gapp->document()){
_te_("Document already released");
        return;
    }
...
NSPoint o=NSMakePoint(0,0);
    o=[self convertPoint:o toView:[self superview]];
    o.x*=2.0;//?
    o.x+=[col frame].origin.x;
    o.x-=_o.x;
    o.y=[col frame].origin.y;
CGPoint location=NSPointToCGPoint(NSViewConvertPointTo(self,o));
long	cmd=popUpContextMenuWithCGPointAndNSWindow(location,[self window],desc,n,0.5);
	if(cmd==0){
		return;
	}
// Replace column
