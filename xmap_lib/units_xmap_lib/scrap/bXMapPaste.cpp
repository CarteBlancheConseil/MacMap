...
void*				cur=data;
void*				last=(void*)((UInt64)data+dtsz);
int					idx,k;
tag*				tg;
...
bEventLog           log(_gapp,this);
	...
	while(cur<last){
		...
		if(tg->sign==kScrapMacMapTagType){
			memcpy(name,(void*)((UInt64)cur+etsz),tg->snsz);
				...
				if(idx!=0){
					...
					switch(k){
						...
						case _ivxs3:
							...
							memcpy(vxs,(void*)((UInt64)cur+etsz+tg->snsz),tg->cnsz);
							vxs=ivs_unpack(vxs);
						...
						default:
							o->setValue(idx,(void*)((UInt64)cur+etsz+tg->snsz));
							break;
					}					
				}
				else{
...
_te_("bad tag sign");
		...
		cur=(void*)((UInt64)cur+etsz+tg->snsz+tg->cnsz);
	}
    _gapp->layersMgr()->SetObjInvalidation(true);
	log.close();
