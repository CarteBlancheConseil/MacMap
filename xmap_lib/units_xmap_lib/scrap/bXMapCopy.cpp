...
_te_("realloc bf(1) failed for "+(unsigned int)vtsz);
		...
		memcpy((void*)((UInt64)bf+bsz),&tg,etsz);
		bsz+=etsz;
		memcpy((void*)((UInt64)bf+bsz),name,tg.snsz);
		bsz+=tg.snsz;
...
_te_("realloc bf(2) failed for "+(unsigned int)vtsz);
			...
			memcpy((void*)((UInt64)bf+bsz),name,tg.snsz);
			bsz+=tg.snsz;
			memcpy((void*)((UInt64)bf+bsz),vbf,tg.cnsz);
			bsz+=tg.cnsz;
		}
	}
...
_bTrace_("bXMapCopy::test",true);
    
_tm_("key "+(void*)[NSApp keyWindow]);
_tm_("main "+(void*)[NSApp mainWindow]);
