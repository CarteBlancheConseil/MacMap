...
#include "CGUtils.h"
#include "bGenericGraphicContext.h"
...
CFStringRef	cfs=CFStringCreateWithCString(kCFAllocatorDefault,font,kCFStringEncodingMacRoman);	
	...
	_lstrt=(long*)malloc(2L*sizeof(long));
	_lstrt[0]=0;
	_lstrt[1]=0;
	_wstrt=(long*)malloc(2L*sizeof(long));
	_wstrt[0]=0;
	_wstrt[1]=0;
...
ByteCount	clen;
	...
	for(long i=0;i<_len;i++){
		...
		if(_text[i]==kMacLineBreak){
			_lstrt[_nlines]=i+1;
			_nlines++;
			_lstrt=(long*)realloc(_lstrt,(_nlines+1)*sizeof(long));
			_lstrt[_nlines]=_len+1;
			
			_wstrt[_nwords]=i+1;
			_nwords++;
			_wstrt=(long*)realloc(_wstrt,(_nwords+1)*sizeof(long));
...
//	clen=strlen(_text)*3+1;
	_utext=(UniChar*)malloc((strlen(_text)+1)*sizeof(UniChar)/**clen*/);
//	CFStringGetCString(cfs,/*(char*)*/_utext,sizeof(UniChar)*clen,kCFStringEncodingUnicode);//
    for(size_t i=0;i<_len;i++){
        _utext[i]=CFStringGetCharacterAtIndex(cfs,i);
    }
//    memcpy(_utext,CFStringGetCharactersPtr(cfs),(CFStringGetLength(cfs)+1)*sizeof(UniChar));// Retourne NULL
    _utext[_len]=0;
	CFRelease(cfs);
}
