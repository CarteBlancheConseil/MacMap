...
//	C-based style wrapper routines around NSMenu
// ------------
long popUpContextMenuWithEventRef(EventRef event,
								  menuitem_desc* desc,
								  long n){
_bTrace_("NSContextualMenuWrappers popUpContextMenuWithEventRef",true);
// Getting WindowRef from EventRef
WindowRef	cbwd=NULL;
	if(GetEventParameter(event,kEventParamWindowRef,typeWindowRef,NULL,sizeof(WindowRef),NULL,&cbwd)!=noErr){
		GetEventParameter(event,kEventParamDirectObject,typeWindowRef,NULL,sizeof(WindowRef),NULL,&cbwd);
	}

// Getting NSWindow from WindowRef ([NSEvent eventWithEventRef:event] don't set the proper window)
NSWindow*	wd=[[[NSWindow alloc] initWithWindowRef:(void*)cbwd] autorelease];
// Getting NSEvent from EventRef (to get the proper location)
NSEvent*	evt=[NSEvent eventWithEventRef:event];
// Building an NSEvent with proper NVWindo and Location
NSEvent*	evt2=[NSEvent otherEventWithType:NSApplicationDefined
	...
	return(popUpContextMenuWithNSEvent(evt2,desc,n));
}
...
NSEvent*	evt= [NSEvent otherEventWithType:NSApplicationDefined
	...
	return(popUpContextMenuWithNSEvent(evt,desc,n));
}
...
long popUpContextMenuWithCGPointAndWindowRef(CGPoint locationInWindow,
											 ...
											 menuitem_desc* desc,
											 long n){
_bTrace_("NSContextualMenuWrappers popUpContextMenuWithCGPointAndWindowRef",true);
NSWindow*	wd=[[NSWindow alloc] initWithWindowRef:(void*)window];
	return(popUpContextMenuWithCGPointAndNSWindow(locationInWindow,wd,desc,n));
}
