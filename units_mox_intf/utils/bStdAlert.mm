...
int ret=[alert runModal];
    ...
    return (1+ret-NSAlertFirstButtonReturn);// Conversion Cocoa button code -> Carbon button code
}
...
bStdAlert::bStdAlert(	int kind, 
						...
						const char* error_string, 
						const char* explanation_string){
    _hit=runAlert(kind,def_btn,use_cancel,error_string,explanation_string,0,nil,nil,nil);
	_res=(_hit==def_btn);
}
...
fprintf(stderr,"%s\n",explanation_string);
    ...
    _hit=runAlert(kind,def_btn,use_cancel,error_string,explanation_string,delay,nil,nil,nil);
	_res=(_hit==def_btn);
}
...
fprintf(stderr,"%s\n",explanation_string);
    ...
    _hit=runAlert(kind,def_btn,use_cancel,error_string,explanation_string,delay,btn1,btn2,btn3);
	_res=(_hit==def_btn);
}
