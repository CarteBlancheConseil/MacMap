...
CFStringRef cfs=(CFStringRef)CFBundleGetValueForInfoDictionaryKey(_bndl,CFSTR("MacMapLibSign"));
char    c[16];
        if(cfs){
            CFStringGetCString(cfs,c,16,kCFStringEncodingMacRoman);
            _sign=*((int*)c);
        }
        else{
            _sign=_kNULLSign_;
...
#endif
_tm_(c+" "+(unsigned int*)(&_family)+"\t"+(unsigned int*)(&lsign)+"\t"+(unsigned int)lsign+" loaded");
// DEBUG>
	}
