...
// 
// -----------
bool bStdUnitMgr::set_current(long idx){
	if((idx>0)&&(idx<=count())){
		_cur=idx;
		return(true);
	}
...
// 
// -----------
bGenericUnit* bStdUnitMgr::get(long idx){
bStdUnit* u;
	if(!_elts.get(idx,&u)){
		u=NULL;
	}
...
// 
// -----------
void bStdUnitMgr::set(long idx, const char* name, double coef){
bGenericUnit* u;
	if(!_elts.get(idx,&u)){
		return;
	}
...
bGenericDocument*		doc=bMacMapApp::instance()->document();
