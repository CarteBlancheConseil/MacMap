...
// 
// -----------
int bTypesMgr::rename(long idx, const char* name){
...
// 
// ------------
bMacMapType* bTypesMgr::get(long idx){
bMacMapType*	tp;
	if(!_elts.get(idx,&tp)){
		return(NULL);
	}
...
// 
// ------------
long bTypesMgr::index(const char* tname){
	if(tname==NULL){
		return(0);
	}
bMacMapType*	tp;
...
// 
// ------------
long bTypesMgr::index(double tid){
bMacMapType*	tp;
long			n=count();
