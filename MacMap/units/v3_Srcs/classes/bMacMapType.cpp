...
// 
// -----------
bool bMacMapType::load_object(long o, bGenericGeoElement** f){
...
// 
// -----------
bool bMacMapType::load_object(long o, bool *b){
...
// 
// -----------
void bMacMapType::inc_sel(long n){
	_nsel+=n;
	if(_nsel<0){
		_nsel=0;
	}
...
// 
// -----------
void bMacMapType::inc_cont(long n){
	_ncnt+=n;
	if(_ncnt<0){
		_ncnt=0;
	}
...
char					val[_values_length_max_];
        ...
        if(!ivr_ok(&_lda)){
_te_("!ivr_ok(&_lda)");
            _lda.left=_lda.top=INT_MIN;
            _lda.right=_lda.bottom=INT_MAX;
        }
