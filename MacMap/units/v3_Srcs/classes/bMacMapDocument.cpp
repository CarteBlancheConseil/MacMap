...
_bTrace_("bMacMapDocument::removeTree",false);
char			path[256];
...
_bTrace_("bMacMapDocument::countParams",false);
bStdDirectory	root(_path);
bStdDirectory	rsrc("Contents/Resources/");
	if(rsrc.status()){
_te_("rsrc.status()="+rsrc.status());
...
_bTrace_("bMacMapDocument::getParamName",false);
bStdDirectory	root(_path);
bStdDirectory	rsrc("Contents/Resources/");
	if(rsrc.status()){
_te_("rsrc.status()="+rsrc.status());
...
_bTrace_("bMacMapDocument::writeXMLParam",false);
bStdDirectory	root(_path);
bStdDirectory	rsrc("Contents/Resources/");
	if(rsrc.status()){
_te_("rsrc.status()="+rsrc.status());
...
_bTrace_("bMacMapDocument::readXMLParam",false);
	(*elt)=NULL;
...
_bTrace_("bMacMapDocument::writeParam",false);
bStdDirectory	root(_path);
bStdDirectory	rsrc("Contents/Resources/");
	if(rsrc.status()){
_te_("rsrc.status()="+rsrc.status());
...
_bTrace_("bMacMapDocument::readParam",false);
	(*buffer)=NULL;
bStdDirectory	root(_path);
bStdDirectory	rsrc("Contents/Resources/");
	if(rsrc.status()){
...
_bTrace_("bMacMapDocument::read_name",false);
int		i,n;
bool	on=false;
...
_bTrace_("bMacMapType::make_package",false);
mode_t	msk=S_IRWXU|S_IRWXG|S_IROTH|S_IXOTH;
...
_bTrace_("bMacMapDocument::pref_load",false);
...
_bTrace_("bMacMapDocument::pref_unload",false);
bStdDirectory	base(_path);
bXMLFile		f(_fpath,"w");
