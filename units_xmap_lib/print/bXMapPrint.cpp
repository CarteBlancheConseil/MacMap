...
OSStatus	DoPrintDialog(	PMPrintSession ps,
    ...
    if(status==noErr){
        status=PMSessionPrintDialog(ps,*pt,pf,&acc);
        if((status==noErr)&&(!acc)){
            status=kPMCancel;
        }
    }
...
PMDestinationType  destTypeP;
        PMSessionGetDestinationType(ps,pt,&destTypeP);
_tm_("destination: "+destTypeP);
...
OSStatus bXMapPrint::print_loop(PMPrintSession ps,
    ...
    // Sauvegarder la zone de dessin de la fenêtre
    //	_gapp->mapwd()->screen_bounds(&back);
    _gapp->mapIntf()->screenBounds(&back);
    ...
    // Début du dessin
    status=PMSessionBeginCGDocument(ps,pt,pf);
    if(status!=noErr){
        _te_("PMSessionBeginCGDocument : "+status);
        return(status);
    }
    ...
    for(i=1;i<=nv;i++){
        ...
        for(j=1;j<=nh;j++){
            ...
            status=PMSessionBeginPage(ps,pf,NULL);
            if(status!=noErr){
                _te_("PMSessionBeginPage : "+status);
                break;
            }
            ...
            CGContextTranslateCTM(ctx,rclip.origin.x,rclip.origin.y);
            _gapp->printMgr()->set_print_context(ctx);
            //			_gapp->layersMgr()->DrawLayers(_gapp->mapwd()->ref(),&cur);
            //			_gapp->layersMgr()->DrawLayers(_gapp->mapintf()->ref(),&cur);
            _gapp->layersMgr()->DrawLayers(NULL,&cur);
            // Signature carto ?
            tempErr=PMSessionEndPage(ps);
            if(status!=noErr){
                status=tempErr;
                _te_("PMSessionEndPage : "+status);
            }
    ...
    // Restaurer le contexte graphique de MacMap
    _gapp->layersMgr()->SwitchContext(kCGGraphicContext,NULL);
    tempErr=PMSessionEndDocument(ps);
    if(status==noErr){
        status=tempErr;
        //_te_("PMSessionEndDocument : %d",status);
    }
    
    tempErr=PMSessionError(ps);
    if(status==noErr){
        status=tempErr;
        //_te_("PMSessionError : %d",status);
    }
