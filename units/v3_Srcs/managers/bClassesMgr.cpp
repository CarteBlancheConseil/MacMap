...
// 
// -----------
bGenericXMLBaseElement* bClassesMgr::NthElement(bGenericXMLBaseElement* root, int idx, const char *name){
giewnPrm	p;
	strcpy(p.name,name);
	p.idx=idx;
	p.n=0;
...
// 
// -----------
int bClassesMgr::CountElements(bGenericXMLBaseElement* root, const char *name){
cewnPrm	p;
	strcpy(p.name,name);
	p.n=0;
