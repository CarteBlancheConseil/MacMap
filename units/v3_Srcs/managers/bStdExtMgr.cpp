...
// 
// -----------
bool bStdExtMgr::event(EventRef evt){
//_bTrace_("bStdExtMgr::event",true);
//_tm_((UInt32*)&_sign);
	if(_closed){
	}
	
	if(!_need_events){
//_tm_("don't need events");
		return(false);
	}
	if(_elts.count()<=0){
//_tm_("count<=0");
...
// 
// -----------
void bStdExtMgr::set_current(int index){
...
// 
// -----------
bGenericExt* bStdExtMgr::get(int idx){
bGenericExtLib*	ext;
...
int	k=_elts.search(&pdummy,comp_name);
...
bDummyExtLib*	pdummy=&dummy;
int	k=_elts.search(&pdummy,comp_signature);
	if(k){
		return(get(k));
	}
	return(NULL);
...
// 
// -----------
void bStdExtMgr::ext_name(int idx, char* name){
bGenericExtLib*	ext;
...
// 
// -----------
UInt32 bStdExtMgr::ext_signature(int idx){
bGenericExtLib*	ext;
...
// 
// -----------
bool bStdExtMgr::remove(int idx){
	return(false);
}
...
// 
// -----------
bool bStdExtMgr::modify(int idx, bGenericXMLBaseElement* root){
	return(false);
}
...
// 
// -----------
int bStdExtMgr::ext_index(const char* name){
bDummyExtLib	dummy(name,0);
bDummyExtLib*	pdummy=&dummy;
...
// 
// -----------
int bStdExtMgr::ext_index(UInt32 signature){
#ifdef __LITTLE_ENDIAN__
	LBSwapWord(&signature,sizeof(UInt32));
#endif
