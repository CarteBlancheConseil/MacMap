...
// 
// -----------
bGenericExt* bStdUserExtMgr::get(int idx){	
...
int				k=_uelts.search(&pdummy,comp_uname);
	if(k){
		if(_uelts.get(k,&ext)){
			return(ext);
		}
...
bStdUserDummy*	pdummy=&dummy;
int				k=_uelts.search(&pdummy,comp_usignature);
	if(k){
		if(_uelts.get(k,&ext)){
			return(ext);
		}
...
// 
// -----------
void bStdUserExtMgr::ext_name(int idx, char* name){
	if(idx<=_elts.count()){
...
// 
// -----------
UInt32 bStdUserExtMgr::ext_signature(int idx){
	if(idx<=_elts.count()){
...
// 
// -----------
bool bStdUserExtMgr::remove(int idx){
...
// 
// -----------
bool bStdUserExtMgr::modify(int idx, bGenericXMLBaseElement* root){
...
// 
// -----------
int bStdUserExtMgr::ext_index(const char* name){
int	k=bStdExtMgr::ext_index(name);
	if(k){
		return(k);
	}
bStdUserDummy	dummy(_MMAPP_,name);
...
// 
// -----------
int bStdUserExtMgr::ext_index(UInt32 signature){
int	k=bStdExtMgr::ext_index(signature);
	if(k){
		return(k);
	}
#ifdef __LITTLE_ENDIAN__
