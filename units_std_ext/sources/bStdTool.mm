...
// 
// -----------
static WindowRef GetWindow(EventRef evt){
WindowRef	wd=NULL;
	if(GetEventParameter(evt,kEventParamWindowRef,typeWindowRef,NULL,sizeof(WindowRef),NULL,&wd)!=noErr){
		GetEventParameter(evt,kEventParamDirectObject,typeWindowRef,NULL,sizeof(WindowRef),NULL,&wd);
	}
	return(wd);
}
...
// 
// -----------
static CGPoint GetMouseClic(EventRef evt){
CGPoint		cgp;
HIRect		hir;
WindowRef	wd=GetWindow(evt);
	GetEventParameter(evt,kEventParamWindowMouseLocation,typeHIPoint,NULL,sizeof(HIPoint),NULL,&cgp);
	if(wd!=NULL){
		if(HIWindowGetBounds(wd,kWindowStructureRgn,kHICoordSpaceWindow,&hir)==noErr){
			cgp.y=hir.size.height-cgp.y;
		}
	}
	return(cgp);
}
...
// 
// -----------
//static int WhichWindow(EventRef evt, bGenericMacMapApp* gapp, int ww){
//WindowRef	wd=GetWindow(evt);
//	if(wd==gapp->mapwd()->ref()){
//		return(1);
//	}
//	return(2);
//}
...
// 
// -----------
//static bGenericExt* getExt(bGenericMacMapApp* gapp){
...
bStdTool::bStdTool(bGenericXMLBaseElement* elt, bGenericMacMapApp* gapp, CFBundleRef bndl) 
	...
	_limit=0.01;
	_angle=45;
	_last_loc.h=LONG_MIN;
	_last_loc.v=LONG_MIN;
	_cur_loc=_last_loc;
	_clic_loc=_last_loc;
	_o=NULL;
	_locs=NULL;
...
//	SetPortWindowPort((WindowRef)(WindowRef)_gapp->mapwd()->ref());
	...
	if(clss==kEventClassWindow){_tm_("kEventClassWindow");
		...
		switch(kind){
			...
			case kEventWindowHandleContentClick:
				GetEventParameter(evt,kEventParamKeyModifiers,typeUInt32,NULL,sizeof(UInt32),NULL,&mod);
				GetEventParameter(evt,kEventParamClickCount,typeUInt32,NULL,sizeof(UInt32),NULL,&count);
				GetEventParameter(evt,kEventParamMouseButton,typeMouseButton,NULL,sizeof(EventMouseButton),NULL,&_btn);
				set_modifiers(mod);
				if(_btn==kEventMouseButtonPrimary){
					cgp=GetMouseClic(evt);
					clic(cgp,count);
				}
				else{
					pop_tools(GetMouseClic(evt),GetWindow(evt));
				}
				b=true;
				break;
			case kEventWindowCursorChange:
//				if(!MMWindowIsActive((WindowRef)_gapp->mapwd()->ref())){
//					break;
//				}
		...
		kind=GetEventKind(evt);
//		_ww=WhichWindow(evt,_gapp,_ww);		
		switch(kind){
			case kEventMouseDragged:_tm_("kEventMouseDragged");
//				if(!MMWindowIsActive((WindowRef)_gapp->mapwd()->ref())){
//					if(_ww!=2){
//						break;
//					}
//				}
				...
				b=true;
				break;
			case kEventMouseEntered:_tm_("kEventMouseEntered");
//				if(!MMWindowIsActive((WindowRef)_gapp->mapwd()->ref())){
//					if(_ww!=2){
//						break;
//					}
//				}
				...
				b=true;
				break;
			case kEventMouseExited:_tm_("kEventMouseExited");
//				if(!MMWindowIsActive((WindowRef)_gapp->mapwd()->ref())){
//					if(_ww!=2){
//						break;
//					}
//				}
				...
				b=true;
				break;
			case kEventMouseDown:_tm_("kEventMouseDown");
//				if(!MMWindowIsActive((WindowRef)_gapp->mapwd()->ref())){
//					if(_ww!=2){
//						break;
//					}
//				}
				...
				//b=true;
				break;
			case kEventMouseUp:_tm_("kEventMouseUp");
//				if(!MMWindowIsActive((WindowRef)_gapp->mapwd()->ref())){
//					if(_ww!=2){
//						break;
//					}
//				}
			...
			case kEventMouseMoved:_tm_("kEventMouseMoved");
				GetEventParameter(evt,kEventParamKeyModifiers,typeUInt32,NULL,sizeof(UInt32),NULL,&mod);
				set_modifiers(mod);
				cgp=GetMouseClic(evt);
				idle(&cgp);
	...
	else if(clss==kEventClassKeyboard){_tm_("kEventClassKeyboard");
		if(_on/*&&MMWindowIsActive((WindowRef)_gapp->mapwd()->ref())*/){
			kind=GetEventKind(evt);
			switch(kind){
...
i2dvertex	vx;
	...
	push();
	if((get_use_angle())&&(is_modifiers(shiftKey))&&(_clic_loc.h!=LONG_MIN)){
...
_tm_(getbundle());
		...
		if(status){
_te_("status="+(int)status+" for CreateNibReferenceWithCFBundle");
			break;;
		}
		status=CreateWindowFromNib(nib,CFSTR("Window"),&_edit_wd);
		if(status){
_te_("status="+(int)status+" for CreateWindowFromNib");
			break;
		}
		hdlr=NewEventHandlerUPP(edit_evt_hdlr);
		if(!hdlr){
_te_("NewEventHandlerUPP returns NULL");
			break;
		}
		InstallWindowEventHandler(_edit_wd,hdlr,GetEventTypeCount(kWindowEvents),kWindowEvents,(void*)this,NULL);
		edit_init(_edit_wd);
		set_arrow();
		ShowWindow(_edit_wd);
		status=RunAppModalLoopForWindow(_edit_wd);
		if(status){
			break;
		}
		break;
...
void bStdTool::constraint_cur(){
	if(is_modifiers(shiftKey)&&(_clic_loc.h!=LONG_MIN)){
i2dvertex	vx;
...
//_bTrace_("bStdTool::set_clic(CGPoint)",true);
		_clic_loc.h=LONG_MIN;
		_clic_loc.v=LONG_MIN;
		return;
	}
...
bStdTool*	t=(bStdTool*)up;
	...
	if(t->edit_event(evt,t->_edit_wd)){
		result=noErr;
	}
	if(GetEventClass(evt)==kEventClassCommand){
		...
		switch(cmd.commandID){
			case kHICommandOK:
				t->save();
				QuitAppModalLoopForWindow(t->_edit_wd);
				result=noErr;
				break;
...
long					nb;
	...
	for(;;){		
		...
		if(root){
			...
			cmd=popUpContextMenuWithCGPointAndWindowRef(location,
														inWindow,
														desc,
														nb);
			if(cmd!=0){
